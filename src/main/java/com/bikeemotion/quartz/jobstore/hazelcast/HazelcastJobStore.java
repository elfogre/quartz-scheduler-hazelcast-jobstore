...
import com.bikeemotion.quartz.jobstore.hazelcast.predicates.TriggerByJobPredicate;
import com.bikeemotion.quartz.jobstore.hazelcast.predicates.TriggersPredicate;
...
public class HazelcastJobStore implements JobStore, Serializable {
    ...
    private IMap<TriggerKey, Object> triggersByKey;
    ...
    @Override
    public void storeJob (final JobDetail job, boolean replaceExisting) throws ObjectAlreadyExistsException, JobPersistenceException {
    ...
    public JobDetail retrieveJob (final JobKey jobKey) throws JobPersistenceException {
        ...
        return jobKey != null && jobsByKey.containsKey(jobKey) ? (JobDetail) jobsByKey.get(jobKey).clone() : null;
    }
    ...
    public void storeTrigger (OperableTrigger trigger, boolean replaceExisting)
        ...
        try {
            ...
            boolean shouldBePaused = pausedJobGroups.contains(newTrigger.getJobKey().getGroup())
            ...
            triggersByKey.set(newTriggerWrapper.getKey(), JsonHelper.toEnvelope(newTriggerWrapper));
            triggersByGroup.put(triggerKey.getGroup(), triggerKey);
        } catch (IOException e) {
            LOG.warn("Error on triggerwrapper to jsonEnvelope.", e);
        } finally {
            try {
    ...
    public OperableTrigger retrieveTrigger (final TriggerKey triggerKey) throws JobPersistenceException {
        ...
        return triggerKey != null && triggersByKey.containsKey(triggerKey)
                ? (OperableTrigger) getTriggerWrapper(triggersByKey.get(triggerKey)).getTrigger().clone() : null;
    }
    ...
    @Override
    public void storeCalendar (final String calName, final Calendar cal, boolean replaceExisting, boolean updateTriggers)
        throws ObjectAlreadyExistsException, JobPersistenceException {
    ...
    public boolean removeCalendar (String calName) throws JobPersistenceException {
        ...
        int numRefs = 0;
        for (Object o : triggersByKey.values()) {
            TriggerWrapper trigger = getTriggerWrapper(o);
            OperableTrigger trigg = trigger.getTrigger();
    ...
    public List<OperableTrigger> getTriggersForJob (final JobKey jobKey) throws JobPersistenceException {
        ...
        return triggersByKey.values(new TriggerByJobPredicate(jobKey)).stream()
                .map(v -> (OperableTrigger) getTriggerWrapper(v).getTrigger()).collect(Collectors.toList());
    }
    ...
    public void pauseTrigger (TriggerKey triggerKey) throws JobPersistenceException {
        ...
        triggersByKey.lock(triggerKey, 5, TimeUnit.SECONDS);
        try {
            final TriggerWrapper newTrigger = newTriggerWrapper(getTriggerWrapper(triggersByKey.get(triggerKey)), PAUSED);
            triggersByKey.set(triggerKey, JsonHelper.toEnvelope(newTrigger));
        } catch (IOException e) {
            LOG.error("Error setting trigger.", e);
        } finally {
            try {
                triggersByKey.unlock(triggerKey);
            } catch (IllegalMonitorStateException ex) {
    ...
    public org.quartz.Trigger.TriggerState getTriggerState (TriggerKey triggerKey) throws JobPersistenceException {
        ...
        try {
            TriggerWrapper tw = getTriggerWrapper(triggersByKey.get(triggerKey));
            if (tw != null) {
    ...
    public void resumeTrigger (TriggerKey triggerKey) throws JobPersistenceException {
        ...
        try {
            if (schedulerRunning) {
                final TriggerWrapper newTrigger = newTriggerWrapper(getTriggerWrapper(triggersByKey.get(triggerKey)), NORMAL);
                triggersByKey.set(newTrigger.getKey(), JsonHelper.toEnvelope(newTrigger));
            }
        } catch (IOException e) {
            LOG.error("Error setting trigger", e);
        } finally {
    ...
    public List<OperableTrigger> acquireNextTriggers (long noLaterThan, int maxCount, long timeWindow) throws JobPersistenceException {
        ...
        List<TriggerWrapper> orderedTriggers = triggersByKey.values(new TriggersPredicate(limit)).stream().map(v -> getTriggerWrapper(v))
        ...
        for (int i = 0; i < orderedTriggers.size(); i++) {
            ...
            triggersByKey.lock(tw.getKey(), 5, TimeUnit.SECONDS);
            try {

                // when the trigger was in acquired state for to much time
                if (tw.getState() == ACQUIRED
                    && (tw.getAcquiredAt() == null || tw.getAcquiredAt() + triggerReleaseThreshold + timeWindow < limit)) {
                    LOG.warn("Found a lost trigger [{}] that should be released at [{}]", tw, limit);
                    releaseAcquiredTrigger(tw.getTrigger());
                    tw = getTriggerWrapper(triggersByKey.get(tw.getKey()));
                }
                ...
                final JobKey jobKey = tw.getTrigger().getJobKey();
                final JobDetail job = jobsByKey.get(tw.getTrigger().getJobKey());
                ...
                OperableTrigger trig = (OperableTrigger) tw.getTrigger().clone();
                ...
                try {
                    triggersByKey.unlock(tw.getKey());
                } catch (IllegalMonitorStateException ex) {
                    LOG.warn("Error unlocking since it is already released.", ex);
                }
            }
    ...
    public List<TriggerFiredResult> triggersFired (List<OperableTrigger> firedTriggers) throws JobPersistenceException {
        ...
        for (OperableTrigger trigger : firedTriggers) {
            ...
            try {
                TriggerWrapper tw = getTriggerWrapper(triggersByKey.get(trigger.getKey()));
                ...
                JobDetail job = retrieveJob(tw.getJobKey());
                ...
                TriggerFiredBundle bndle = new TriggerFiredBundle(retrieveJob(tw.getJobKey()), trigger, cal, false, new Date(),
    ...
    private ArrayList<TriggerWrapper> getTriggerWrappersForJob (JobKey jobKey) {
        ...
        triggersByKey.values().stream().filter( (trigger) -> (getTriggerWrapper(trigger).getJobKey().equals(jobKey)))
                .forEach( (trigger) -> {
                    trigList.add(getTriggerWrapper(trigger));
                });
    ...
    private boolean applyMisfire (TriggerWrapper tw) throws JobPersistenceException {
        ...
        this.schedSignaler.notifyTriggerListenersMisfired((OperableTrigger) tw.getTrigger().clone());

        tw.getTrigger().updateAfterMisfire(cal);

        if (tw.getTrigger().getNextFireTime() == null) {
            storeTriggerWrapper(newTriggerWrapper(tw, STATE_COMPLETED));
            schedSignaler.notifySchedulerListenersFinalized(tw.getTrigger());

        } else if (tnft.equals(tw.getTrigger().getNextFireTime())) {
            return false;
        }
    ...
    private boolean removeTrigger (TriggerKey key, boolean removeOrphanedJob) throws JobPersistenceException {
        ...
        try {
            final TriggerWrapper tw = getTriggerWrapper(triggersByKey.remove(key));
            removed = tw != null;
            if (removed) {
                ...
                if (removeOrphanedJob) {
                    JobDetail job = jobsByKey.get(tw.getJobKey());
    ...
    private void storeTriggerWrapper (final TriggerWrapper tw) {
        ...
        try {
            triggersByKey.set(tw.getKey(), JsonHelper.toEnvelope(tw));
        } catch (IOException e) {
            LOG.error("Error setting param", e);
        }
    }

    /**
     * Set the max time which a acquired trigger must be released. It should be > 30000, since quartz executes acquireNextTriggers in a 30000 interval
     * 
    ...
    private TriggerWrapper getTriggerWrapper (Object o) {
        if (o == null) {
            return null;
        }
        JsonEnvelope envelope = (JsonEnvelope) o;
        try {
            return JsonHelper.toValueType(envelope.getJsonValue(), TriggerWrapper.class);
        } catch (IOException e) {
            LOG.error("Error casting object to TriggerWrapper via JsonEnvelope.", e);
            return null;
        }
    }
